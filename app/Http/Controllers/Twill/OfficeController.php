        ...
        return parent::getForm($model)
            ->addFieldset(
                Fieldset::make()->title('Content')->fields([
                    ...
                    Medias::make()->name('office')->label('Office images')->max(12)->minWidth(1500)->note('여러 장의 이미지를 추가할 수 있습니다.'),
                    Input::make()->name('email')->label('Email')->type(Input::TYPE_EMAIL),
                    Input::make()->name('phone')->label('Phone number')->note('국가번호, 공백, 하이픈을 포함해 자유롭게 입력할 수 있습니다.'),
                    ...
                    Select::make()->name('timezone')->label('Timezone')->placeholder('시간대 선택')->searchable()->clearable()->options(
